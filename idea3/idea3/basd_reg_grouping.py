"""Scale-density groups and lagged regression-error state for BASD-Reg."""
...
class BASDRegGroupState(nn.Module):
    ...
    Membership depends only on ground-truth geometry. Instance weights are
    computed from the *previous* regression-error EMA, and ``update`` is called
    only after the current loss has been formed.
    """
    ...
    def compute_weights(self, membership: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # Writing the interpolation around one makes alpha=0 and equal group
        # errors exactly equivalent to the unweighted O2 reduction, rather
        # than relying on a floating-point membership row sum being exactly 1.
