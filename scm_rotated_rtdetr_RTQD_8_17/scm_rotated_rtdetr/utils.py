...
def soft_ar_membership(
    aspect_ratio: Tensor,
    boundaries=(2.0, 4.0, 8.0),
    temperature: float = 0.20,
    eps: float = 1e-6,
) -> Tensor:
    """Soft AR membership:
    ...
    if len(boundaries) != 3:
        raise ValueError(
            'AR boundaries must contain exactly 3 values.'
        )
    ...
    if not (1.0 < b1 < b2 < b3):
        raise ValueError(
            f'Invalid AR boundaries: {boundaries}'
        )

    log_ar = torch.log(
        aspect_ratio.clamp_min(1.0 + eps)
    )

    log_boundaries = torch.log(
        aspect_ratio.new_tensor(
            [b1, b2, b3]
        )
    )
