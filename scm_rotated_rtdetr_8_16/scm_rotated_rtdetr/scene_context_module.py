...
class SceneContextModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_ar_groups = int(num_ar_groups)
        self.ar_bias_scale = float(ar_bias_scale)
        ...
        self.ar_prior_table = nn.Parameter(
            torch.zeros(
                num_scene_prototypes,
                self.num_ar_groups,
            )
    ...
    def forward(self,
        ...
        bounded_ar_bias = (
            self.ar_bias_scale
            * torch.tanh(raw_ar_logits)
        )

        bounded_ar_bias = (
            bounded_ar_bias
            - bounded_ar_bias.mean(
                dim=-1,
                keepdim=True,
            )
        )
        ...
        return dict(
            ...
            raw_ar_logits=raw_ar_logits,
            ar_bias=ar_bias,
            ar_prior_table=self.ar_prior_table,
