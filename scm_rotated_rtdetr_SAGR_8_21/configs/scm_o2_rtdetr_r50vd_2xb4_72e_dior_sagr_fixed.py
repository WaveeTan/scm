...
from projects.rotated_dino.rotated_dino.match_cost import (
    ChamferCost,
    GDCost,
)
...
# Keep this consistent with your AutoDL environment.
data_root = "/root/autodl-tmp/datasets/DIOR/"
...
model = dict(
    ...
    bbox_head=dict(
        # ----------------------------------------------------
        # ONLY structural change from 8/15:
        # RotatedRTDETRHead -> SCMSAGRRotatedRTDETRHead
        # The original O² losses remain unchanged; SAGR is auxiliary
        # and is applied only to final matching decoder queries.
        # ----------------------------------------------------
        type=SCMSAGRRotatedRTDETRHead,
        ...
        # ----------------------------------------------------
        # Keep original L1Loss.
        #
        # IMPORTANT:
        # This is still the ORIGINAL O² L1 loss and remains active
        # for encoder, all decoder matching layers and DN queries.
        # ----------------------------------------------------
        loss_bbox=dict(
            type=L1Loss,
            loss_weight=5.0,
        ...
        # ----------------------------------------------------
        # NEW: final-decoder-only Scale-Aspect Geometry Residual.
        #
        # This DOES NOT replace loss_bbox. The original O² L1 term above
        # remains active for encoder, every decoder layer, final decoder,
        # and DN queries. SAGR adds one small residual term only to the
        # final matching decoder layer.
        # ----------------------------------------------------
        sagr_loss_cfg=dict(
            # Residual fades to zero at scale >= 0.04.
            scale_ref=0.04,

            # Elongated supervision starts at AR=4 and reaches full
            # strength at AR=8.
            ar_ref=4.0,
            ar_full=8.0,
            ...
            # Current O² angle is normalized by pi, so normalized period=1.
            angle_period=1.0,
        ),
