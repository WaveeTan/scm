...
class TestRTPModules(unittest.TestCase):
    ...
    def test_scne_multihead_pooling_mask_and_gradients(self):
        ...
        scene_logits = head(memory, memory_mask)
        self.assertEqual(tuple(scene_logits.shape), (2, 4))
        self.assertEqual(tuple(head.token_score.weight.shape), (4, 16))
        scene_logits.sum().backward()
        for parameter in head.parameters():
            self.assertIsNotNone(parameter.grad)

    def test_scne_rejects_fully_masked_memory(self):
        head = self.scene_type(embed_dims=8, num_classes=3)
        memory = torch.randn(1, 4, 8)
        with self.assertRaisesRegex(ValueError, "hide every token"):
            head(memory, torch.ones(1, 4, dtype=torch.bool))
